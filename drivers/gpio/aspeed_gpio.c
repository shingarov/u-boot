...
/*
 * GPIO driver for the ASPEED SoC
 *
...
static int aspeed_gpio_direction_input(struct udevice *dev,
				       unsigned int offset)
{
...
static int aspeed_gpio_direction_output(struct udevice *dev,
					unsigned int offset,
					int value)
{
	struct aspeed_gpio_priv *priv = dev_get_priv(dev);
...
static int aspeed_gpio_set_value(struct udevice *dev, unsigned int offset,
				 int value)
{
	struct aspeed_gpio_priv *priv = dev_get_priv(dev);
	const struct aspeed_gpio_bank *bank = to_bank(offset);
...
U_BOOT_DRIVER(gpio_aspeed) = {
	...
	.priv_auto_alloc_size = sizeof(struct aspeed_gpio_priv),
};
