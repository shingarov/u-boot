...
static u32 ast2600_get_uart_clk_rate(struct ast2600_scu *scu, int uart_idx)
{
	...
	case 6:
		...
		break;
	case 5: /* 24mhz comes from usb phy 48Mhz */
		{
		u8 uart5_clk_sel = 0;
		/* high bit */
...
static u32 ast2600_configure_ddr(struct ast2600_scu *scu, ulong rate)
{
	...
	mpll.out = rate;
	if (!ast2600_search_clock_config(&mpll)) {
...
static u32 ast2600_configure_mac12_clk(struct ast2600_scu *scu)
{
	...
	epll.out = 1000000000;
	if (!ast2600_search_clock_config(&epll)) {
		printf("error!! unable to find valid ETHNET MAC clock setting\n");
				 ...
				 struct ast2600_mac_clk_div *p_cfg)
{
	...
	pll.out = p_cfg->fin;
	if (!ast2600_search_clock_config(&pll)) {
		printf("error!! unable to find valid ETHNET MAC clock setting\n");
				   ...
				   struct ast2600_mac_clk_div *p_cfg)
{
	...
	if (p_cfg->src == ASPEED_CLK_EPLL || p_cfg->src == ASPEED_CLK_HPLL) {
		/*
		 * Re-init PLL if the current PLL output frequency doesn't
		 * match the divider setting
		 */
	...
	reg_350 &= ~BIT(31);
#if 0
	if (p_cfg->rgmii_3_4_clk_src == RGMII_3_4_CLK_SRC_HCLK)
		reg_350 |= BIT(31);
...
static ulong ast2600_enable_extsdclk(struct ast2600_scu *scu)
{
	...
	/*
	 * Default use apll for clock source 800/4 = 200.
